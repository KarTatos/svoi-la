  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", height:"calc(100vh - 120px)" }}>
      ...
      {!user ? (
        ...
        {chat.length<=1 && <div style={{ marginBottom:12 }}><div style={{ display:"flex", flexWrap:"wrap", gap:6 }}>{["Грин-карта через брак?","Стоимость N-400?","Работа без EAD?","Статус кейса?"].map((s,i)=><button key={i} onClick={()=>onSend(s)} style={pl(false)}>{s}</button>)}</div></div>}
