...
export default function PhotoViewerModal({
  ...
  const photos = Array.isArray(photoViewer.photos) ? photoViewer.photos : [];
  const currentPhoto = photos[photoViewer.index];
  ...
  return (
    <div onClick={onClose} style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.82)", zIndex:300, display:"flex", alignItems:"center", justifyContent:"center", padding:16, touchAction:"none" }}>
      ...
      {photos.length > 1 && (
        <>
          <button onClick={(e)=>{e.stopPropagation(); if (typeof onPrev === "function") onPrev();}} style={{ position:"absolute", left:10, top:"50%", transform:"translateY(-50%)", width:42, height:42, borderRadius:"50%", border:"1px solid rgba(255,255,255,0.35)", background:"rgba(0,0,0,0.35)", color:"#fff", fontSize:22, cursor:"pointer", zIndex:4 }}>‹</button>
          <button onClick={(e)=>{e.stopPropagation(); if (typeof onNext === "function") onNext();}} style={{ position:"absolute", right:10, top:"50%", transform:"translateY(-50%)", width:42, height:42, borderRadius:"50%", border:"1px solid rgba(255,255,255,0.35)", background:"rgba(0,0,0,0.35)", color:"#fff", fontSize:22, cursor:"pointer", zIndex:4 }}>›</button>
        </>
      )}
      <div
