...
export default function MarketDetailModal({
  item, isLiked, canEdit, T, cd, pl,
  onClose, onToggleLike, onShare, onEdit, onDelete,
}) {
    ...
    >
      ...
      >
        ...
        <div style={{ position: "relative", width: "100%", paddingTop: "75%", background: T.bg, borderRadius: "22px 22px 0 0", overflow: "hidden" }}>
          {photos.length > 0 ? (
            <Image src={photos[photoIdx]} alt={item.title} fill sizes="(max-width: 480px) 100vw, 480px" style={{ objectFit: "cover" }} />
          ) : (
          ...
          <button
            onClick={onClose}
            style={{ position: "absolute", top: 14, left: 14, background: "rgba(0,0,0,0.45)", border: "none", borderRadius: "50%", width: 34, height: 34, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", color: "#fff", fontSize: 20, lineHeight: 1 }}
          >‹</button>
          ...
          <div style={{ display: "flex", gap: 6, padding: "10px 14px 0", overflowX: "auto" }}>
            {photos.map((p, i) => (
              <div
                ...
                style={{ position: "relative", width: 52, height: 52, flexShrink: 0, borderRadius: 8, overflow: "hidden", border: `2px solid ${i === photoIdx ? T.primary : "transparent"}`, cursor: "pointer" }}
              >
                <Image src={p} alt="" fill sizes="52px" style={{ objectFit: "cover" }} />
              </div>
            ))}
          </div>
        )}
