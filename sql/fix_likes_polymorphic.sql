-- Fix likes schema to match app code:
-- likes(item_id, item_type, user_id) + automatic likes_count sync
-- Safe to run multiple times.

BEGIN;

-- Ensure likes_count exists on all content tables
...
-- Add required columns if table existed in old shape
...
-- Migrate old place_id -> item_id/item_type if needed
DO $$
BEGIN
  IF EXISTS (
    SELECT 1
...
-- Make sure required columns are not nullable after migration
UPDATE public.likes
...
-- Normalize item_type values and keep only allowed values
UPDATE public.likes
...
-- Drop old place-based unique constraints/indexes if present
DO $$
DECLARE c record;
BEGIN
  FOR c IN
...
-- Rebuild likes_count from likes table
...
-- Trigger: keep likes_count in sync for all item types
...
-- RLS policies for polymorphic likes
